...
def _base_graph():
    return {
        "nodes": [{"id": "0", "metadata": {"type": "cluster", "rank": -1,
...
def test_populate_is_idempotent():
    g = _base_graph()
    g["nodes"].append({"id": "1", "metadata": {"type": "qdevice_ibm", "rank": -1,
                                               "paths": {"containment": "/c0/qdevice_ibm0"}}})
    g["edges"].append({"source": "0", "target": "1", "metadata": {"subsystem": "containment"}})
    h = _FakeHandle(g)
    added = qgraph.populate(h, ["ibm"])
